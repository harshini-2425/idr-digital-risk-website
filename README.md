...
flowchart TD
    A[Hero] --> B[About IDR]
    B --> C[Service Pillars]
    C --> D[Train-Hire-Innovate-Deploy Pipeline]
    D --> E[Community / Who We Serve]
    E --> F[Contact / Register Interest]
```
...
```mermaid
flowchart LR
    A[Academy] --> B[Innovation and Incubation]
    B --> C[Advisory Services]
    C --> D[Deployment Impact]
```
...
```mermaid
flowchart TD
    U1[Visitor Lands on Hero] --> U2[Understands IDR Mission]
    U2 --> U3[Explores About and Service Pillars]
    U3 --> U4[Identifies Relevant Audience Path]
    U4 --> U5[Registers Interest via Contact Form]
```
